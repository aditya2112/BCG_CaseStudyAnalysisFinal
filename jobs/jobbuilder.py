    ...
    def extractionData(self, questionId:int):
        try:
            if questionId == None:
                ...
                self.logger._logger.info("Extraction->9")
                tempDataframe: DataFrame = self.load.readCsvFile(os.path.join(os.path.join(self.PARENTDIR, self.config['resource']['source_path']), settings.QUESTION_MAPPING["9"][0]))
                self.logger._logger.info("Processing->9")
                tempDataframe: DataFrame = self.spark.createDataFrame([self.cityWiseAccident(personDataframe=tempDataframe)], "integer").toDF("NO_OF_CITY_WISE_ACCIDENTS")
                questionFileName: str = "Question_9"
            
            elif questionId == 10:
                self.logger._logger.info("Extraction->10")
                rightDataframe: DataFrame = self.load.readCsvFile(os.path.join(os.path.join(self.PARENTDIR, self.config['resource']['source_path']), settings.QUESTION_MAPPING["10"][0]))
                leftDataframe: DataFrame = self.load.readCsvFile(os.path.join(os.path.join(self.PARENTDIR, self.config['resource']['source_path']), settings.QUESTION_MAPPING["10"][1]))
                self.logger._logger.info("Processing->10")
                tempDataframe: DataFrame = self.spark.createDataFrame([self.timeOfDayAccidents(unitsDataframe=leftDataframe, personDataframe=rightDataframe)], "integer").toDF("NO_OF_ACCIDENTS_BY_TIME_OF_DAY")
                questionFileName: str = "Question_10"
